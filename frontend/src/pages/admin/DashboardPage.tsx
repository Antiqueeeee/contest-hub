...
import { Flag, ClipboardList, BarChart3, Newspaper, ArrowRight, Plus } from 'lucide-react'
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    Promise.all([
      api.get<{ items: any[] }>('/admin/contests'),
      api.get<{ items: any[]; total: number }>('/admin/registrations'),
      api.get<{ items: any[]; total: number }>('/admin/results'),
      api.get<{ items: any[]; total: number }>('/admin/news'),
    ]).then(([c, r, rs, n]) => {
      setContests(c.items); setRegTotal(r.total); setResultTotal(rs.total); setNewsTotal(n.total)
  ...
  const stats = [
    ...
    { label: '已发布成绩', value: resultTotal, sub: '条记录', icon: BarChart3 },
    { label: '新闻总数', value: newsTotal, sub: '篇', icon: Newspaper },
  ]
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">首页概览</h1><p className="text-muted-foreground text-sm mt-0.5">竞赛管理数据中心</p></div>
        <Link to="/admin/contests/new" className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg text-sm font-medium hover:bg-primary/90 transition-colors no-underline"><Plus className="h-4 w-4" />创建赛事</Link>
      </div>
          ...
          <Card key={s.label} className="border-0 shadow-sm overflow-hidden">
            ...
            <CardHeader className="pb-1 pt-3">
              <div className="flex items-center justify-between">
                ...
                <div className={`p-2 rounded-lg bg-gradient-to-r ${statsGradients[i].from} ${statsGradients[i].to}`}><s.icon className="h-4 w-4 text-white" /></div>
              </div>
            </CardHeader>
            <CardContent><div className="text-2xl font-bold">{s.value}</div><p className="text-xs text-muted-foreground mt-0.5">{s.sub}</p></CardContent>
          </Card>
        ))}
      </div>
      <div className="grid grid-cols-3 gap-4">
        <Card className="border-0 shadow-sm">
          ...
          <CardContent className="space-y-3">
            {(['open', 'ongoing', 'draft', 'finished'] as const).map(status => {
              ...
              const pct = Math.round((count / (contests.length || 1)) * 100)
              ...
              return (
                <div key={status} className="space-y-1">
                  <div className="flex justify-between text-sm"><span className="text-muted-foreground">{labels[status]}</span><span className="font-medium">{count}</span></div>
                  <div className="h-1.5 bg-muted rounded-full overflow-hidden"><div className={`h-full rounded-full ${barColors[status]}`} style={{ width: `${pct}%` }} /></div>
                </div>
              )
            })}
          </CardContent>
        ...
        <Card className="border-0 shadow-sm col-span-2">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Link to="/admin/contests" className="text-sm text-primary hover:underline flex items-center gap-1 no-underline">查看全部 <ArrowRight className="h-3 w-3" /></Link>
          </CardHeader>
