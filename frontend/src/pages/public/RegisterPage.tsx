...
export default function RegisterPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await api.post<{ registration_number: string }>(`/public/contests/${contest.id}/register`, {
        contest_id: contest.id, group_id: groupId ? Number(groupId) : null, name, phone, custom_fields: cf, privacy_agreed: privacyAgreed,
      })
    ...
    } catch (e) { alert(e instanceof Error ? e.message : '报名失败') } finally { setSubmitting(false) }
  }
    ...
    <div className="max-w-lg mx-auto">
      ...
      <Card><CardHeader><CardTitle className="text-lg">{contest.title}</CardTitle><div className="flex items-center gap-1 text-sm text-muted-foreground"><Clock className="h-3 w-3" />报名截止：{contest.registration_end?.split('T')[0]}</div></CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-1"><Label>姓名 <span className="text-destructive">*</span></Label><Input value={name} onChange={e => { setName(e.target.value); setErrors({}) }} placeholder="请输入真实姓名" />{errors.name && <p className="text-sm text-destructive">{errors.name}</p>}</div>
          <div className="space-y-1"><Label>手机号 <span className="text-destructive">*</span></Label><Input value={phone} onChange={e => { setPhone(e.target.value); setErrors({}) }} placeholder="请输入 11 位手机号" maxLength={11} />{errors.phone && <p className="text-sm text-destructive">{errors.phone}</p>}</div>
          ...
          <div className="flex items-start gap-2 pt-2"><Checkbox id="privacy" checked={privacyAgreed} onCheckedChange={v => { setPrivacyAgreed(!!v); setErrors({}) }} /><Label htmlFor="privacy" className="text-sm text-muted-foreground cursor-pointer">我已阅读并同意《隐私政策》，同意平台收集和使用我的个人信息用于本次赛事报名。</Label></div>
