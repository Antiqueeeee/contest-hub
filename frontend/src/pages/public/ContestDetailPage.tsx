...
export default function ContestDetailPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {/* Header */}
      <div className="mb-8">
        <Badge className={`mb-4 text-sm px-3 py-1 ${statusCfg[contest.status]?.cls}`}>
          {statusCfg[contest.status]?.label}
        </Badge>
        <h1 className="text-3xl font-bold mb-4">{contest.title}</h1>
        <div className="flex flex-wrap gap-2 text-sm text-muted-foreground">
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-muted rounded-lg">
            <Calendar className="h-4 w-4 text-primary" />{contest.startDate} ~ {contest.endDate}
          </span>
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-muted rounded-lg">
            <MapPin className="h-4 w-4 text-primary" />{contest.location}
          </span>
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-muted rounded-lg">
            <Users className="h-4 w-4 text-primary" />{contest.maxParticipants ? `限 ${contest.maxParticipants} 人` : '人数不限'}
          </span>
          {isOpen && (
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 text-amber-700 rounded-lg">
              <Clock className="h-4 w-4" />截止 {contest.registrationEnd.split('T')[0]}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex gap-3 mb-8">
        {isOpen && (
          <Link to={`/contests/${contest.id}/register`}>
            <Button size="lg" className="h-12 px-8 text-base border-0 shadow-lg" style={ctaGradient}>
              立即报名
            </Button>
          </Link>
        )}
        {isFinished && (
          <Link to={`/contests/${contest.id}/results`}>
            <Button size="lg" className="h-12 px-8 border-0 shadow-lg" style={ctaGradient}>
      ...
      <Card className="border-0 shadow-sm mb-6">
        ...
        <CardContent>
          <div className="text-foreground/80 leading-relaxed text-sm" dangerouslySetInnerHTML={{ __html: contest.description }} />
        </CardContent>
      </Card>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {groups.length > 0 && (
          <Card className="border-0 shadow-sm">
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2"><Layers className="h-4 w-4 text-primary" />参赛组别</CardTitle>
            </CardHeader>
        ...
        {awardsList.length > 0 && (
          <Card className="border-0 shadow-sm">
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2"><Award className="h-4 w-4 text-primary" />奖项设置</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {awardsList.map((a, i) => (
