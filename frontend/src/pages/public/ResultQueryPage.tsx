          ...
          {queried && found && (
            <div className="mt-4 p-4 bg-muted rounded-lg space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">奖项</span><span className="font-medium text-primary">{found.award_name}</span></div>
              </div>
            </div>
          )}
          {queried && !found && !error && !querying && <p className="text-sm text-muted-foreground text-center py-4">未查询到成绩。请检查报名编号和手机号是否正确，或成绩尚未发布。</p>}
        </CardContent></Card>
    </div>
  )
}
