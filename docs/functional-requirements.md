...
一个平台内可以有多个管理员账号，按角色分权：

| 角色 | 说明 | 典型权限 |
|------|------|----------|
| 超级管理员 | 平台最高权限 | 所有功能 + 管理其他管理员账号 |
| 管理员 | 赛事运营人员 | 新闻管理 + 赛事管理 + 报名管理 + 成绩管理 + 数据导出 |
| 编辑员 | 内容编辑 | 仅新闻管理（编辑/发布/分类） |
...
系统部署后通过数据库脚本或命令行创建一个初始超级管理员账号。后续所有管理员由超级管理员在后台创建，不开放公开注册。

#### 3.1.2 管理员列表（仅超级管理员可见）
...
| 操作 | 说明 | 权限 |
|------|------|------|
| 添加管理员 | 弹窗填写用户名/密码/姓名/角色 | 仅超级管理员 |
| 编辑 | 修改姓名、角色 | 仅超级管理员 |
| 禁用/启用 | 禁止该账号登录 | 仅超级管理员 |
| 重置密码 | 设为随机密码并提示修改 | 仅超级管理员 |
...
| 页面 | 路由 | 说明 | 可见角色 |
|------|------|------|:--------:|
| 登录页 | /admin/login | 账号密码登录 | 所有人 |
| 首页 | /admin | 赛事数量概览 | 管理员+ |
| 管理员管理 | /admin/users | 增删改管理员 | 仅超级管理员 |
| 新闻列表 | /admin/news | 表格 + 搜索过滤 | 所有人 |
| 新闻编辑 | /admin/news/:id | 富文本编辑 | 所有人 |
| 新闻分类 | /admin/news/categories | 增删改查 | 所有人 |
| 赛事列表 | /admin/contests | Tab 切换状态 | 管理员+ |
| 赛事创建/编辑 | /admin/contests/new, /admin/contests/:id | 分步表单 | 管理员+ |
| 报名列表 | /admin/registrations | 按赛事查看 | 管理员+ |
| 成绩列表 | /admin/results | 按赛事查看，Tab 切换 | 管理员+ |
| 成绩录入 | /admin/results/:id/edit | 弹窗逐条录入 | 管理员+ |
| 数据导出 | /admin/export | 选择类型 → 赛事 → 字段 | 管理员+ |
...
| PUT | /api/admin/users/:id | 编辑管理员角色/姓名 |
