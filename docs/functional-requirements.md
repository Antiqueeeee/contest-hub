...
| 角色 | 说明 | 权限范围 |
|------|------|----------|
| 选手（已注册） | 参赛选手 | 浏览赛事、提交报名、查询个人成绩、查看历史参赛记录 |

选手使用手机号注册账号，登录后参赛。报名时自动关联到选手账号，免去重复填写基本信息。选手中心可查看跨赛事的历史报名和成绩记录。
...
- 入口：前台页面顶部导航栏「登录 / 注册」
- 方式：手机号 + 密码注册（MVP 不做短信验证码）
- 注册表单：手机号、姓名、密码（6-20 位）
- 注册后自动登录，跳转到首页

#### 3.2.2 登录

- 手机号 + 密码登录
- 登录后导航栏显示选手姓名，可进入「我的」页面
- 支持退出登录
...
| 页面 | 内容 |
|------|------|
| 我的报名 | 跨赛事查看所有报名记录，显示赛事名称、报名编号、报名时间、组别 |
| 我的成绩 | 跨赛事查看所有已发布的个人成绩，总分、排名、奖项 |
| 个人信息 | 编辑姓名、手机号 |

#### 3.2.4 报名关联

- 选手登录后报名，系统自动将报名记录关联到选手账号（通过手机号匹配或登录状态）
- 选手未登录时也可以报名，但提示登录后可查看历史记录
- 历史报名记录可通过手机号匹配后合并到选手账号
...
平台介绍、常见问题、联系我们等静态页面内容，管理员可在后台直接编辑，前台动态渲染，无需修改代码重新部署。

#### 3.9.2 后台页面

| 字段 | 类型 | 说明 |
|------|------|:--:|
| 页面标识 | 下拉选择 | about / faq / contact |
| 内容 | 富文本编辑器 | 支持 HTML 格式 |
...
- `/about` → 查询 page_key="about" 的内容渲染
- `/faq` → 查询 page_key="faq" 的内容渲染
- `/contact` → 查询 page_key="contact" 的内容渲染
...
erDiagram
    ...
    Registration {
        bigint id PK
        bigint contest_id FK
        bigint contestant_id FK "可空，选手登录后关联"
...
| 选手注册 | /register | 手机号 + 密码注册 |
| 选手登录 | /login | 手机号 + 密码登录 |
| 报名表单 | /contests/:id/register | 动态表单（登录后自动填入基本信息） |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | /api/auth/contestant/register | 选手注册（手机号+密码+姓名） |
| POST | /api/auth/contestant/login | 选手登录（手机号+密码，返回 JWT） |
| GET | /api/contestant/profile | 获取选手个人信息 |
| PUT | /api/contestant/profile | 编辑选手个人信息 |
| GET | /api/contestant/registrations | 我的报名记录 |
| GET | /api/contestant/results | 我的成绩记录 |
...
- **选手账号管理**（手机号注册登录 + 选手中心：我的报名/我的成绩/个人信息）
- **站点内容管理**（后台编辑平台介绍、常见问题、联系我们等静态页面内容，前台动态渲染，无需改代码）
...
| 需求 | 说明 | 实现方式 |
|------|------|----------|
| **传输加密** | 全站强制 HTTPS，所有前后端交互加密传输 | Nginx 配置 SSL 证书，HTTP → HTTPS 301 重定向 |
| **密码存储** | 管理员密码不得明文存储 | bcrypt / argon2 单向哈希，加盐 |
| **敏感字段加密** | 选手手机号、身份证号等个人信息在数据库中加密存储，不能明文落库 | AES-256 应用层加密，密钥从环境变量注入，不与代码一同提交 |
| **数据脱敏** | 后台列表页手机号默认脱敏显示 | 中间 4 位显示为 `****`，如 `138****1234` |
| **导出安全** | Excel 导出文件含个人完整信息（手机号明文），仅通过认证链接下载，7 天后自动删除 | JWT 校验 + 一次性下载 Token + 定时清理 |
| **日志脱敏** | 日志中不得记录手机号、密码等敏感信息的明文 | 日志中间件自动过滤敏感字段 |
| **JWT 安全** | Token 设置合理的过期时间，支持主动失效 | Access Token 有效期 2h，Refresh Token 7d；退出登录时加入黑名单 |

### 8.2 接口安全

| 需求 | 说明 |
|------|------|
| **身份认证** | 除前台公开接口外，所有管理后台 API 必须携带有效 JWT，中间件拦截校验 |
| **防暴力破解** | 登录接口同一 IP 1 分钟内最多 5 次失败尝试，超出后锁定 15 分钟 |
| **防刷限流** | 报名提交：同一 IP 1 分钟 3 次；成绩查询：同一手机号 1 分钟 5 次 |
| **输入校验** | 所有接口入参必须做服务端二次校验（不信任前端校验），防止恶意请求 |
| **SQL 注入** | ORM 参数化查询（SQLAlchemy），禁止拼接 SQL |
| **XSS 防护** | 富文本内容（新闻正文、赛事介绍）输出时做 HTML 清洗，过滤 script/iframe 等危险标签 |
| **CORS** | 仅允许白名单域名跨域访问 API |
| **文件上传** | 图片上传限制类型（jpg/png/webp）、大小（≤ 5MB），后端校验文件头魔数 |

### 8.3 审计日志

| 日志类型 | 记录内容 | 保留时长 |
|----------|----------|:--------:|
| 登录日志 | 谁、何时、IP、成功/失败 | 90 天 |
| 操作日志 | 谁、何时、做了什么操作（删除报名/发布成绩/导出数据/编辑赛事等） | 180 天 |
| 导出日志 | 谁、何时、导出了哪个赛事的数据（报名/成绩）、条数 | 180 天 |

日志不可被管理员在后台删除，仅可通过数据库层面管理。

### 8.4 数据备份

| 需求 | 说明 |
|------|------|
| **数据库备份** | 每日自动全量备份，保留最近 7 天 |
| **备份加密** | 备份文件加密存储，不与数据库同机存放 |
| **恢复验证** | 每季度执行一次备份恢复演练，确保备份可用 |
| **导出文件清理** | 定时任务清理超过 7 天的导出文件（本地磁盘 + 数据库记录） |

### 8.5 性能

| 指标 | 目标值 | 说明 |
|------|--------|------|
| 报名提交响应 | P95 < 2s | 含表单校验 + 数据库写入 |
| 成绩查询响应 | P95 < 1s | 读多写少，加 Redis 缓存 |
| 页面首屏加载 | < 3s | 前台 H5 页面 |
| 导出吞吐 | 单次 ≤ 50,000 条，生成耗时 < 5min | 异步任务，不阻塞请求 |
| 并发报名 | 支持同一赛事 100 人同时提交报名 | 数据库连接池 + 乐观锁防超卖 |

### 8.6 合规（个人信息保护）

| 需求 | 说明 |
|------|------|
| **隐私政策** | 报名表单页须展示隐私政策勾选框，未勾选不可提交 |
| **最小化收集** | 仅收集赛事必需的个人信息，不索要与赛事无关的数据 |
| **数据删除** | 选手可联系管理员要求删除个人报名信息；后台提供「删除个人信息」操作，删除后匿名化处理 |
| **数据留存** | 软删除的报名数据 30 天后自动物理删除；正常数据保留至赛事归档后 1 年 |
| **Cookie 声明** | 前台页面如使用 Cookie（如 Session），须在页面底部展示 Cookie 声明 |

### 8.7 可用性

| 指标 | 目标值 |
|------|--------|
| 服务可用率 | 99.5%（月度，不含计划维护） |
| 维护窗口 | 提前 24h 通知，尽量安排在凌晨 2:00-4:00 |
| 异常恢复 | 服务异常宕机后 5 分钟内自动重启（Docker restart policy） |

### 8.8 部署与环境

| 需求 | 说明 |
|------|------|
| **容器化部署** | Docker Compose 一键启动（app + DB + Redis + Nginx） |
| **配置分离** | 数据库密码、加密密钥、JWT Secret 等敏感配置通过环境变量注入，不硬编码，不提交到 Git |
| **环境区分** | 至少区分开发环境（本地）和生产环境，使用不同的配置文件 |
| **健康检查** | 应用暴露 `/api/health` 端点供监控探测 |
| **日志收集** | 应用日志输出到 stdout/stderr，由 Docker 日志驱动收集 |

### 8.9 多终端适配

| 需求 | 说明 |
|------|------|
| 前台移动端 | 选手报名、查成绩页面须适配主流手机屏幕（320px ~ 428px），表单和按钮触控友好 |
| 前台 PC 端 | 浏览器兼容 Chrome/Firefox/Edge/Safari 最近两个大版本 |
| 管理后台 | 仅适配 PC 端（1280px+），推荐 Chrome 最新版 |
